...
int get_file_list(const char *dir) {
  ...
  while((de = readdir(dr)) != NULL) {
    ...
    if(S_ISDIR(buffer.st_mode) && is_listed_directory(file)) {
      ...
      *(files + i) = file;
      i++;
    } else if(strcmp(file, "main.cpp") != 0) {
      char file_end[5] = {
        *(file + filename_length - 4),
        *(file + filename_length - 3),
        *(file + filename_length - 2),
        *(file + filename_length - 1),
        '\0'
      };

      if(strcmp(file_end, ".cpp") == 0) {
        *(file + filename_length - 4) = '\0';
        *(files + i) = file;
        i++;
      }
    }
...
int is_listed_directory(const char *dir) {
  // *** When updating this array, don't forget to update array size and loop max 'i' below. ***
  char unlisted_dirs[8][50] = {
    ".",
    "..",
    ".cache",
    ".ccls-cache",
